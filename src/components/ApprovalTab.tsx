...
export function ApprovalTab() {
  ...
  const selectedReq = requests.find((r) => r.requestId === selected);
          ...
          >
                ...
                {breakGlassMode && (
                  <div className="bg-purple-50 border border-purple-200 rounded-md p-3 space-y-2">
                    ...
                    <p className="text-xs text-purple-600">
                      Bypasses normal approval flow. This action is logged as a non-deletable
                      audit entry and will appear in compliance reports.
                    </p>
                    <button
                      onClick={() => resolve(req.requestId, 'break_glass', 'break-glass-operator')}
                      className="px-3 py-1.5 bg-purple-600 text-white text-xs rounded-md hover:bg-purple-700 transition-colors"
                    >
                      Confirm break-glass override
                    </button>
                  </div>
                )}
              </div>
            ...
            {selected === req.requestId && req.status !== 'pending' && req.resolvedBy && (
              <div className="mt-4 pt-4 border-t border-gray-200">
                ...
                {req.justification && (
                  <p className="text-xs text-gray-500 mt-1 italic">&ldquo;{req.justification}&rdquo;</p>
                )}
              </div>
            )}
          </div>
